...
public class LoanCalc {
	...
	public static double bisectionSolver(double loan, double rate, int n, double epsilon) {
		...
		while (Math.abs(H - L) > epsilon) {
			iterationCounter += 1;
			if (endBalance(loan, rate, n, g) * endBalance(loan, rate, n, L) > 0) { // the soultion must be between g And
				// H.
				L = g;
				g = (L + H) / 2;
	...
	private static double endBalance(double loan, double rate, int n, double payment) {
		for (int i = 0; i < n; i++) {
			loan = (loan - payment) * (1 + rate / 100); // calculate how much left in the loan after one period.
		}
		return loan;
	}
}